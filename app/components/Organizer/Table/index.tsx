...
import { PencilIcon, TrashIcon, ChevronUpIcon, ChevronDownIcon, EyeIcon } from '@heroicons/react/24/outline';
          ...
          {sortedData.map((row, rowIndex) => (
            <tr key={rowIndex} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
              ...
              <td className="px-4 py-2 text-sm text-text-dark dark:text-text-light flex">
                <EyeIcon className="w-4 h-4 mt-2 text-text-light dark:text-text-dark cursor-pointer hover:text-accent dark:hover:text-accent" />
                <TrashIcon className="w-4 h-4 mt-2 ml-2 text-text-light dark:text-text-dark cursor-pointer hover:text-red-500 dark:hover:text-red-500" />
              </td>
            </tr>
          ))}
        </tbody>
